...
const CategorysListPage = ({categorys, setCategorys, isMock, setIsMock}:Props) => {
    ...
    const {category_name} = useAppSelector((state:RootState) => state.categorys)
    ...
    const fetchCategorys = async () => {
        try {
            ...
            const response = await fetch(`${apiUrl}/api/categorys/?category_name=${category_name.toLowerCase()}`)
    ...
    const fetchCartData = async () => {
        try {
            const env = await import.meta.env;
            const apiUrl = isTauri() ? env.VITE_API_URL : ""
            await fetch(`${apiUrl}/api/imts/cart/`)
        } catch {
            createMocks()
        }
    }
