...
import {AsyncThunkConfig, T_CartDataResponse, T_CategoryUpdateValue} from "modules/types"
...
export const updateCategoryValue = createAsyncThunk<void, T_CategoryUpdateValue, AsyncThunkConfig>(
    ...
    async function({category_id, weight},thunkAPI) {
        const state = thunkAPI.getState()
        if (state.imts.imt) {
            await api.imts.imtsUpdateCategoryUpdate(state.imts.imt.id.toString(), category_id.toString(), {weight})
        }
    }
)
